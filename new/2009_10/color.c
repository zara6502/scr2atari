    ...
    unsigned level_count)
{
    ...
    for (y = 0; y < COLOR_LOGICAL_HEIGHT; ++y)
    {
        unsigned x;
        ...
        for (x = 0; x < COLOR_LOGICAL_WIDTH; ++x)
        {
            ...
            offset =
                (size_t)y * COLOR_LOGICAL_WIDTH + x;
            ...
            for (i = 0; i < level_count; ++i)
            {
                ...
                if (i + 1 < level_count)
                {
                    ...
                    if (value >= mixed)
                        mixed_distance = value - mixed;
                    else
                        mixed_distance = mixed - value;

                    if (mixed_distance < best_distance)
                    {
                        best_distance = mixed_distance;
                        best_index = i * 2 + 1;
                    }
                }
            }

            image[offset] = (uint8_t)best_index;
        }
    }
}
    ...
    unsigned *unique_colors)
{
    ...
    /*
     * Render directly to the physical 320x240 image.
     *
     * Solid level:
     *
     *     G G
     ...
     *     B A
     *
     * The 2x2 checkerboard therefore consists of real
     * square physical pixels.
     */
    for (y = 0;
         y < COLOR_PHYSICAL_HEIGHT;
         ++y)
