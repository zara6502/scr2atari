...
static void quantize_grey_mix_levels(
    uint8_t *image,
    ...
    unsigned level_count)
{
    ...
    /*
     * Each logical pixel is classified independently.
     *
     * Possible classes:
     *
     *     0 = G0
     ...
     * IMPORTANT:
     *
     * This function does NOT create a checkerboard.
     * It only decides which pixels belong to a MIX class.
     */
    for (y = 0;
         ...
         ++y)
    {
             ...
             ++x)
        {
            ...
            /*
             * Try virtual MIX levels between adjacent
             * grayscale levels.
             *
             * MIX(Gi, Gi+1) has the average luminance
             * of the two real levels.
             */
            for (i = 0;
                 i + 1 < level_count;
                 ++i)
            {
                ...
                if (value >= mixed)
                    distance = value - mixed;
                else
                    distance = mixed - value;
             ...
             *
             * The MIX itself is materialized later,
             * on the 160x240 logical grid.
             */
            image[offset] =
                (uint8_t)best_index;
        }
...
static void materialize_grey_mix_levels(
    uint8_t *image,
    const unsigned levels[COLOR_MAX_COLORS],
    unsigned level_count)
{
    ...
    /*
     * Materialize MIX classes on the FINAL LOGICAL
     * 160x240 coordinate system.
     *
     * The checker phase is global:
     *
     *     ((x ^ y) & 1) == 0
     *
     * selects the first/lower color.
     *
     *     ((x ^ y) & 1) == 1
     *
     * selects the second/upper color.
     *
     * There is NO 2x2 block processing here.
     */
    for (y = 0;
         y < COLOR_LOGICAL_HEIGHT;
         ++y)
    ...
    unsigned *unique_colors)
{
    ...
    /*
     * Select the four smart grayscale levels from
     * the original 256x192 ZX image.
     */
    level_count =
        build_smart_grey_levels(
            scr,
     ...
     *     160 x 240
     *
     * We keep luminance here until the final rendering
     * stage.
     */
    luminance =
        (uint8_t *)malloc(
     ...
     *
     * IMPORTANT:
     * No dithering and no quantization happens here.
     */
    for (y = 0;
         y < COLOR_LOGICAL_HEIGHT;
         ++y)
    ...
    /*
     * Quantize the 160x240 luminance image to:
     *
     *     G0
     *     MIX(G0,G1)
     *     G1
     ...
     *     G3
     *
     * There is no error diffusion and no Bayer matrix.
     */
quantize_grey_mix_levels(
    luminance,
    levels,
    level_count);

/*
 * Materialize all virtual MIX colors on the
 * final 160x240 logical image.
 *
 * After this point luminance[] contains only
 * real grayscale level indices 0..3.
 */
materialize_grey_mix_levels(
    luminance,
    levels,
    level_count);

/*
 * Render the already materialized 160x240 logical image.
 *
 * At this point luminance[] contains ONLY real grayscale
 * level indices:
 *
 *     0 = G0
 *     1 = G1
 ...
 *     3 = G3
 *
 * There is NO color mixing here.
 *
 * The physical 320x240 image is simply:
 *
 *     logical pixel -> two identical horizontal pixels
 *
 * Therefore:
 *
 *     A B C
