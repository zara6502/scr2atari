...
struct OptimizerRoundStats {
    size_t parentCount = 0;
    size_t candidatePairs = 0;
    size_t shannonCandidates = 0;
    size_t exactChildren = 0;

    uint64_t bestChildBytes = UINT64_MAX;
    uint64_t bestChildPayloadBits = UINT64_MAX;

    uint64_t bestBeforeBytes = UINT64_MAX;
    uint64_t bestAfterBytes = UINT64_MAX;
};
 ...
 *     compressed bytes
 */
struct ExactSizeBreakdown {
    uint64_t headerBytes = 0;
    uint64_t grammarBytes = 0;
    uint64_t tableHeaderBytes = 0;
    uint64_t tableBytes = 0;
    uint64_t bitCountBytes = 0;
...
ExactSizeBreakdown calculateExactSingleFileSize(
    uint32_t grammarCount,
    const std::vector<uint8_t>& tableData,
    uint64_t compressedBytes)
{
    ExactSizeBreakdown result;

    result.headerBytes = 40;

    result.grammarBytes =
        static_cast<uint64_t>(grammarCount) * 4ULL;

    result.tableHeaderBytes = 4;

    result.tableBytes =
        static_cast<uint64_t>(tableData.size());

    result.bitCountBytes = 8;
    ...
    result.totalBytes =
        result.headerBytes +
        result.grammarBytes +
        result.tableHeaderBytes +
        result.tableBytes +
        result.bitCountBytes +
        result.compressedBytes;
...
uint64_t exactSingleFileSize(
    ...
    uint64_t compressedBytes)
{
    return calculateExactSingleFileSize(
        grammarCount,
        tableData,
    ...
    const OptimizerState& state)
{
    ...
    if (!evaluateExactState(
        ...
        std::fprintf(
            stderr,
            "%s: evaluateExactState FAILED\n",
            label);

        return;
    }

    const ExactSizeBreakdown breakdown =
        calculateExactSingleFileSize(
            state.dictionary.size(),
            exact.tableData,
            exact.compressedBytes);
    ...
    std::fprintf(
        stderr,
        "header            : %" PRIu64 "\n",
        breakdown.headerBytes);

    std::fprintf(
        stderr,
        "grammar           : %" PRIu64 "\n",
        breakdown.grammarBytes);

    std::fprintf(
        stderr,
        "table header      : %" PRIu64 "\n",
        breakdown.tableHeaderBytes);

    std::fprintf(
        stderr,
        "table             : %" PRIu64 "\n",
        breakdown.tableBytes);

    std::fprintf(
        stderr,
        "bit count         : %" PRIu64 "\n",
        breakdown.bitCountBytes);
...
std::vector<OptimizerState> generateChildren(
    const OptimizerState& parent,
    OptimizerRoundStats& diagnostics)
{
    std::vector<OptimizerState> result;
    ...
    std::vector<OptimizerCandidate>
        candidates =
            evaluateCandidates(
                parent);
    static int debugRound = 0;

    if (debugRound < 10) {
        std::fprintf(
            stderr,
            "    candidates=%zu parent_total=%llu parent_payload=%llu rules=%u\n",
            candidates.size(),
    ...
    for (size_t candidateIndex = 0;
        ...
        if (candidateIndex < 5) {
            ...
            std::fprintf(
                stderr,
                "      cand[%zu] pair=(%u,%u) repl=%u "
                "shannon=%.2f -> total=%llu payload=%llu "
                "delta=%lld\n",
                candidateIndex,
                candidate.left,
                candidate.right,
                candidate.replacementCount,
                candidate.shannonCost,
                static_cast<unsigned long long>(
                    child.totalBytes),
                static_cast<unsigned long long>(
                    child.payloadBits),
    ...
    if (exactImproved != 0 ||
        ...
        std::fprintf(
            stderr,
            "      exact: improved=%zu worse=%zu total=%zu\n",
            exactImproved,
            exactWorse,
...
printExactStateBreakdown(
    "OPTIMIZER INITIAL",
    initial);

const uint64_t initialTotalBytes =
    initial.totalBytes;

const uint64_t initialPayloadBits =
    initial.payloadBits;

const uint32_t initialRules =
    initial.dictionary.size();

const size_t initialTokens =
    ...
    std::fprintf(
        stderr,
        "\n"
        "=== GTC optimizer diagnostic ===\n"
        "initial: total=%" PRIu64
        " bytes, payload=%" PRIu64
        " bits, rules=%u, tokens=%zu\n",
        best.totalBytes,
        best.payloadBits,
    ...
    for (int round = 0;
        ...
        std::fprintf(
            stderr,
            "\nROUND %d: beam=%zu best_total=%llu best_payload=%llu\n",
            round,
            beam.size(),
            static_cast<unsigned long long>(
                best.totalBytes),
        ...
        diagnostics.parentCount =
            beam.size();

        diagnostics.bestBeforeBytes =
            best.totalBytes;

        const auto roundStart =
            Clock::now();
        ...
        std::fprintf(
            ...
            children.size());
        /*
         * Find the best child BEFORE beam pruning.
         *
         * This is the most important diagnostic value.
         */
        for (const OptimizerState& state :
             children) {

            if (state.totalBytes <
                    diagnostics.bestChildBytes ||
                (state.totalBytes ==
                     diagnostics.bestChildBytes &&
                 state.payloadBits <
                     diagnostics.bestChildPayloadBits)) {
        ...
        for (size_t i = 0;
            ...
            std::fprintf(
                stderr,
                "    beam[%zu]: total=%llu payload=%llu rules=%u\n",
                i,
                static_cast<unsigned long long>(
                    next[i].totalBytes),
                static_cast<unsigned long long>(
                    next[i].payloadBits),
                next[i].dictionary.size());
        }
        ...
        const double roundMs =
            std::chrono::duration<double, std::milli>(
                roundEnd - roundStart).count();
        ...
        std::fprintf(
            ...
            best.sequence.size(),
            next.size(),
            hasNewGrammar ? "yes" : "no",
            roundMs);

        /*
         * Additional useful diagnostic:
         *
         * If childBest is already smaller than globalBest before
         * selectBeam(), the problem is in beam selection.
         */
        if (diagnostics.bestChildBytes <
            diagnostics.bestBeforeBytes) {
    ...
    const auto optimizerEnd =
        Clock::now();

    const double optimizerMs =
        std::chrono::duration<double, std::milli>(
            optimizerEnd - optimizerStart).count();

const bool improved =
    best.totalBytes < initialTotalBytes ||
    (best.totalBytes == initialTotalBytes &&
     best.payloadBits < initialPayloadBits);
 ...
 * compare the physical file size with the optimizer's
 * calculated size.
 */
std::ifstream verify(
    outputName,
...
if (verify) {
    ...
    const std::streamoff physicalSize =
        verify.tellg();
    ...
    std::fprintf(
        stderr,
        "stats compressed  : %" PRIu64 " bytes\n",
        stats.compressedBytes);

    std::fprintf(
        stderr,
        "stats payload     : %" PRIu64 " bits\n",
        stats.compressedBits);
