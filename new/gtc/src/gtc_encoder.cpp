    ...
    std::vector<uint32_t>& sequence)
{
     ...
     * 256+   = grammar rules
     *
     * Each iteration finds the most frequent non-overlapping
     * adjacent token pair and replaces it with a new token.
     *
     * This is deliberately simple. Later versions can replace
     * this builder with a much more sophisticated optimizer.
     */

    while (sequence.size() >= 2) {
        std::unordered_map<uint64_t, PairInfo> pairs;

        pairs.reserve(sequence.size());
        ...
        for (const auto& item : pairs) {
            const PairInfo& info = item.second;
        ...
        /*
         * Three occurrences is the minimum useful threshold
        ...
        next.reserve(sequence.size() - best.count);
    ...
    GtcEncodeStats& stats)
{
    const uint32_t tokenCount =
        GTC_BASE_TOKENS + dictionary.size();

    std::vector<uint64_t> frequencies(tokenCount, 0);
    ...
    huffman.encode(sequence, writer);
    ...
    header.magic = GTC_MAGIC;
    header.version = GTC_VERSION;
    header.original_size = originalSize;
    header.token_count = tokenCount;
    header.grammar_count = dictionary.size();
    header.token_count_in_stream = sequence.size();
    ...
    for (uint32_t i = 0; i < dictionary.size(); ++i) {
        ...
        writeU32(out, rule.left);
        writeU32(out, rule.right);
    }
    ...
    stats.finalTokenCount = sequence.size();
    stats.tokenCount = tokenCount;
    stats.grammarCount = dictionary.size();
    stats.compressedBits = writer.bitCount();
