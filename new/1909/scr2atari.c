 ...
 * scr2atari.c
 *
 * ZX Spectrum .SCR -> Atari XL/XE XEX / BIN / PNG converter.
 *
 * Supports:
...
static int rgb_luminance(uint8_t r, uint8_t g, uint8_t b)
{
    return
        (2126 * (int)r +
         7152 * (int)g +
                        ...
                        Mutex *print_mutex)
{
    ...
    if (strcmp(format, "png") == 0)
    {
        ...
        result =
            write_png(output_name, rgb, 256, 192);
    ...
    else if (strcmp(format, "color") == 0)
    {
         ...
         * 192x192; each logical pixel occupies two horizontal
         * RGB pixels in the reference image.
         */
